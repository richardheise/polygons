...
vector<Polygon> readPolygons(uint m, bool verbose) {
    ...
    for (uint i = 0; i < m; i++) {
        ...
        if (verbose) {
            cout << "Polígono " << i + 1 << " tem " << verticesNum << " vértices." << endl;
        }
        ...
        while (verticesNum--) {
            ...
            if (verbose) {
                cout << "Vértice adicionado: (" << newDot.x << ", " << newDot.y << ")" << endl;
            }
        }
    }
...
vector<Dot> readDots(uint n, bool verbose) {
    ...
    for (uint i = 0; i < n; i++) {
        ...
        if (verbose) {
            cout << "Ponto adicionado: (" << newDot.x << ", " << newDot.y << ")" << endl;
        }
    }
...
// Função para imprimir os polígonos (modo verboso)
...
// Função para imprimir os pontos (modo verboso)
